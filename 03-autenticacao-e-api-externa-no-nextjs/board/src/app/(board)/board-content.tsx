...
import { ArchiveIcon, ThumbsUpIcon, MessageCircleIcon } from "lucide-react";
import z from "zod";
...
export function BoardContent({ issues }: BoardContentProps) {
    const { backlog, todo, in_progress, done } = issues;
    const allIssuesIds = [...backlog, ...todo, ...in_progress, ...done].map(issue => issue.id);
    ...
    const interactions = interactionsData && new Map<string, { isLiked: boolean; likesCount: number; }>(interactionsData.interactions.map(interaction => [
        interaction.issueId,
        {
